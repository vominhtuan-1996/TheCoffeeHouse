...
@interface CakesViewController ()<CollectionViewCellDelegate,UIPopoverPresentationControllerDelegate>
...
- (NSUInteger)checkCoreData {
    ...
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"type = %@",@"Cakes"];
...
//    [cell setDataWithImage:[UIImage imageNamed:self.arrayImage[indexPath.row]] infoLabel:self.arrayInfo[indexPath.row] priceLabel:self.arrayPrice[indexPath.row]];
    [cell setDataWithObject:self.arrayNew[indexPath.row] type:@"Cakes"];
    return cell;
...
- (void)setData {
    ...
    self.arrayInfo = [NSMutableArray arrayWithObjects:
                      ...
                      @"Bánh Fudgy dark",
                      @"Bánh Delish Instant Pot Peanut Butter Explosion",
    ...
    for (int i = 0; i < self.arrayImage.count; i++){
        ...
        [newData setValue:@"Cakes" forKey:@"type"];
        [newData setValue:[NSNumber numberWithInteger:i + (self.arrayImage.count * 2)] forKey:@"idFood"];
    }
